...
function changeSeycAll() {
    sync = !sync;
    var syncAllButtem = document.getElementById("syncAll")
    if (sync == true) {
        syncAllButtem.style.color = "#9999ff"
        syncAllButtem.textContent = "sync all flights"
    }
    if (sync == false) {
        syncAllButtem.style.color = "#ff3300"
        syncAllButtem.textContent = " NOT sync all flights"
    }
}
